...
const salesController = {
    ...
    printCustomerBalance: async(req, res) => {
        try {
            ...
            const drawHeader = () => {
                let currentX = startX;
                const headerData = ['Cantidad', 'Producto', 'Precio'];
    ...
    //get customer positive balance
    positiveBalance: async(req, res) => {
        try {

            const { id_customers, date_from } = req.query
            const filters = {}
            ...
            //get data
            const data = await transactionsQueries.resumedTransactions({ filters })

            //get positive balance
            const notAssignedPayments = data.filter( d => d.type == 'PAGO NO ASIGNADO')[0]?.totalAmount || 0
            const refunds = data.filter( d => d.type == 'REINTEGRO')[0]?.totalAmount || 0
            const assignations = data.filter( d => d.type == 'ASIGNACION')[0]?.totalAmount || 0

            const positiveBalance = notAssignedPayments - refunds - assignations
    ...
    lastOrders: async(req, res) => {
        try {
            ...
            //add filters
            if (customers) {
                filters.customers = filters.customers = JSON.parse(customers)
            }

            //get data
            const data = await ordersQueries.lastOrders({ filters })
